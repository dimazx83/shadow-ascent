...
Production is deployed by Vercel from the GitHub repository. Do not edit
`dist/` directly; it is generated by `npm run build` and ignored by git.

Standard update flow:

```bash
# 1. Make code or asset changes locally
npm run build
...
Vercel auto-deploys after the push. Wait for the deployment to finish in the
Vercel dashboard, then open the installed PWA on iPhone. The service worker is
configured with `autoUpdate`, so new app code should be picked up without
clearing IndexedDB data.

### Preserving PWA Data

Player stats, quests, habits, inventory, and dungeon progress live in IndexedDB
on the device. Normal GitHub/Vercel deploys do **not** wipe this data.

Do not casually change these if preserving existing iPhone data matters:
- Dexie database name: `new Dexie('shadow_ascent')`
- Table names or primary keys in `src/js/modules/db.js`
- `reset_all_data()` behavior
- PWA `start_url` / `scope` / production domain, unless a migration plan exists

These actions can clear or strand local data:
- Tapping Stats → Danger Zone → Reset All Data
- Deleting the Home Screen app / clearing Safari website data
- Changing to a different production domain/origin
- Renaming the IndexedDB database

If a database schema change is needed, add a new Dexie version migration instead
of clearing tables.
